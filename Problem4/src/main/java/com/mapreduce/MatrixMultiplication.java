    ...
    public static class Map extends Mapper<LongWritable, Text, Text, Text> {
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (fileName.equals("file1")) {
                int i = Integer.parseInt(indicesAndValue[0]);
                ...
                for (int kPerV = 0; kPerV < pPerV; kPerV++) {
                    ...
                    outputValue.set("A," + Integer.toString(i % s) + "," + Integer.toString(j % t) + "," + indicesAndValue[2]);
                    context.write(outputKey, outputValue);
                }
            } else {
                int j = Integer.parseInt(indicesAndValue[0]);
                int k = Integer.parseInt(indicesAndValue[1]);
                for (int iPerS = 0; iPerS < mPerS; iPerS++) {
                    outputKey.set(Integer.toString(iPerS) + "," + Integer.toString(j / t) + "," + Integer.toString(k / v));
                    outputValue.set("B," + Integer.toString(j % t) + "," + Integer.toString(k % v) + "," + indicesAndValue[2]);
                    context.write(outputKey, outputValue);
                }
            }
        }
